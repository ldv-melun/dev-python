...
En python les chaines de caractères sont instances du type `str`, et sont des séquences ordonnées de valeurs (notées en hexadécimal) de point de code UNICODE.
...
Seuls les champs renseignés lors de cette seconde saisie seront mis à jour.
...
Voici un programme python faisant usage de caractères unicode non conventionnels, n'ayant pas toujours un glyphe spécifique associé !! C'est une technique d'*obfuscation de code source*, bien connue des hackers malveillants.
...
. Contrôler vos connaissances sur https://quizbe.org/question?id-selected-topic=6[quizbe.org]. (choisir `PYTHON-1`, scope `tp2`)
. Proposer, pour le thème `PYTHON-1`, scope `tp2`, 2 questions QCM originales et personnelles, sur des thèmes couverts pas cette séquence d'exercices.
