...
Ces deux classes disposent des opérations sur les ensembles bien pratiques telles que : l'*union*, l'*intersection*, la *différence* de 2 ensembles. Voir ici  https://docs.python.org/fr/3/library/stdtypes.html#set.
...
[TIP]
====
Attention, dans le cas des dictionnaire, cette structure retournera les clés, pas les valeurs.
...
. Définir une fonction nommé `test_recherche_elements_communs` qui teste plusieurs scénarios (inspirez-vous de l'exemple donnée et de `test_note()`)
...
----
doublons(['1', 'X','2','2','3','4','3', 'A' 'a', 'X']) # ['X', '2', '3']
----
...
. Concevoir une nouvelle fonction qui prend en argument une liste et qui détermine si cette liste est sans doublon ou non.
